...
[Identity evidence](24-sprint-1-evidence.md) adds local authentication/MFA/offboarding, nine new tenant-table isolation checks, permission/concurrency tests, frontend browser flows and authenticated TLS SMTP delivery tests. P21 remains OPEN for controlled external mailbox receipt. Sprint 1 run 34436595874 executed and failed migration checksum validation; corrective hosted verification is pending. The Sprint 0 baseline has actual attempt-3 hosted success; only its historical attempt 1 failed before repository steps due to billing. Neither local SMTP acceptance nor passing local tests is a hosted CI pass or production release approval; Gates B/C/D remain CLOSED.
