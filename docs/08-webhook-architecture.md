...
Proposed verification contract: GET checks `hub.mode`, a constant-time comparison with stored verify-token digest, and returns `hub.challenge` only when valid. POST verifies `X-Hub-Signature-256` as HMAC-SHA256 over exact raw bytes using the app secret; constant-time compare; no decode/reencode before verification. **Primary Meta reference/rotation behavior remains unavailable (M26); the owner approves strict implementation using M56/M57 first-party evidence, with live proof required for Sprint 2 acceptance.** Never implement a permissive fallback. Verify token is not the POST signing secret.
...
## Sprint 2 implemented security boundary

The owner's Gate B clarification authorizes implementation from inspected first-party examples and verified account topology; live GET/POST proof remains mandatory for acceptance. Runtime uses file-backed distinct App Secret and Verify Token, no unsigned flag. Exact raw byte HMAC, strict single header/prefix/digest parsing, constant-time comparison and strict GET duplicate-parameter rejection are covered by tests.
