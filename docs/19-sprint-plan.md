...
| 2 — Meta ingestion and existing assets | Conditional after Gate B: protected Graph boundary, WABA/phone/profile sync, subscription visibility, signed durable ingress, classification/dedupe/retry/DLQ/replay, Meta Health and event diagnostics | Gate B APPROVED, real account inventory and controlled webhook evidence, all new-table RLS/concurrency tests, own green hosted commit; no outbound, full Inbox persistence or media-download implementation |
...
Sprint 1 implementation is authorized and covers the identity/access scope only. [Executed evidence and architecture](24-sprint-1-evidence.md) track completion. Acceptance is COMPLETE after operator confirmation of all four controlled email receipts. Sprint 0 hosted verification now has actual attempt-3 success. Sprint 1 is published at d4fed8c; run 34436595874 failed the repository migration checksum check. Corrective commit 9b2c671 passed the complete hosted workflow in run 34437626714; the historical account restriction is not its cause. Sprint 2 remains conditional on Gate B; no Phase B work has started. Gates B/C/D remain CLOSED.

## Phase A execution status

[Gate B evidence](25-gate-b-evidence.md) records official-source failures and remaining company-account verification. Phase A is INCOMPLETE; Gate B CLOSED. [Sprint 2 evidence](26-sprint-2-evidence.md) records NOT STARTED rather than an invented implementation result. Phase B is permitted only after all mandatory dependencies pass. No outbound WhatsApp is authorized in either phase; no Sprint 3 work.

The 2026-09-11 continuation is Gate B only: v26.0 is a candidate for read-only account probes, not automatic approval. Stop for separate evidence review even after Gate B passes; no Sprint 2 implementation in this run. The supplied config is now present and limited v26.0 GETs pass; verified SYSTEM_USER and remaining empty Verify Token/current contract blockers are recorded in doc 25.
