...
Sprint 1 implementation is authorized and covers the identity/access scope only. [Executed evidence and architecture](24-sprint-1-evidence.md) track completion. Acceptance is OPEN pending controlled SMTP receipt evidence. Sprint 0 hosted verification now has actual attempt-3 success. Sprint 1 is published at d4fed8c; run 34436595874 failed the repository migration checksum check. Corrective hosted verification is pending; the historical account restriction is not its cause. Do not start Sprint 2. Gates B/C/D remain CLOSED.
