...
Development prerequisites and checks are in [README](../README.md). Run python scripts/dev.py up, then python scripts/dev.py migrate. Migration 00002 creates identity/access tables and requires the dedicated waba_identity role provisioned by up. Schema version 2 is required. Use a reviewed forward migration for repairs; do not edit published SQL or regenerate a missing root key.
...
Controlled mailbox evidence remains PENDING - EXTERNAL CREDENTIAL BLOCKER. The user supplied relay configuration and a designated mailbox on 2026-09-10, but the attempted real SMTP test was rejected with SMTP_AUTH before any message was sent. Local TLS SMTP tests are not a substitute. Once available, use the implemented application/outbox to deliver an Owner verification, employee invitation, password reset and security-change notice to controlled addresses. Confirm actual receipt and successful explicit completion; verify old/superseded links fail and no email exposes passwords, session tokens, TOTP seeds or recovery codes. Record UTC time, application delivery ID, sanitized Message-ID, purpose, SMTP result, receipt confirmation and any bounce in the Sprint 1 evidence record. Store no secret proof or recipient address in the repository. This evidence does not require or authorize Meta messaging.
...
Hosted GitHub CI is a separate verification track. After the account restriction is resolved, rerun the workflow for the reviewed implementation commit, inspect actual job steps/results, record the URL and commit, and fix any repository failures. Do not turn an external attempted failure into a hosted pass.
