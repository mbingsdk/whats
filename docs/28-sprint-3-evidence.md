...
## Historical bounded-cost acceptance review, 2026-09-16
...
Hosted CI for this correction: PASS for [commit fe9a6a7](https://github.com/mbingsdk/whats/commit/fe9a6a7bf7994edec56644a4f39c3e2706979be6), [run 35051420281](https://github.com/mbingsdk/whats/actions/runs/35051420281), completed 2026-09-16. The exact published commit passed every required stage: isolated PostgreSQL, migration/repeat verification, source/SQL/Go/real PostgreSQL/frontend/OpenAPI checks, identity/Meta/Inbox browser flows, dependency integrity/vulnerability checks and cleanup. This is the correction's own hosted result, not the earlier baseline. This evidence-only follow-up leaves Sprint 3 implementation COMPLETE, acceptance OPEN, paid/template authority CLOSED and Gate D CLOSED. The remaining acceptance sequence is full executable pricing coverage across the verified 30-day horizon, bounded maximum exposure approval and controlled live evidence. Automatic review rejected a proposed AGENTS.md edit, so that instruction file was left unchanged; current owner decisions are recorded in the authorized project documents instead.


## Controlled executable pricing review, 2026-09-18

Sprint 3 implementation COMPLETE; acceptance OPEN; paid/template authority CLOSED; Gate D CLOSED. Scope is pricing coverage only. No callback, tunnel, Meta challenge, fresh inbound request, send intent or provider request was performed. Live provider attempts remain 0.

Planning interval: 2026-09-18T10:02:48Z through 2026-10-18T10:02:48Z (30 days). October transition: 2026-10-01T07:00:00Z for the reviewed America/Los_Angeles WABA basis. [M74](21-research-register.md#m74-executable-idr-service-candidate-review-2026-09-18) records retained source hashes/timestamps and the finite quarterly-coverage rationale.

The approved limited correction adds source candidate SPRINT3_IDR_SERVICE_20260918 with pre-October zero-policy coverage and October IDR Service list coverage through 2027-01-01T08:00:00Z exclusive. The candidate ignores unverifiable allowance/discounts. Its offline conservative maximum is IDR 356.65000000 for one delivered Service TEXT; it is neither an actual charge nor an authorized live quote. A successful published-coverage preflight can propose that exact one-message TEST amount, with a maximum 60-second quote expiry; it activates no budget. No owner financial approval has been requested or invented.

Real Registry audit used the existing authenticated Owner session and scoped API on loopback, with Meta and live acceptance disabled. Result: no rate imports, no rate publications, runtime currency UNKNOWN, one active organization member. The operator-designated reviewer account is not a member of this local organization. The current Owner is unverified and MFA is disabled. Consequently no real DRAFT/APPROVED/PUBLISHED state was fabricated, no second account was impersonated, and no currency activation or publication was performed. The operator's IDR fact remains accepted; recording it in runtime now has a permission/MFA-protected API.

Full real coverage: BLOCKED. Publication IDs: NONE. Active TEST budget: NONE. Owner approval: NOT GIVEN. Registry prerequisites are a verified/MFA-enabled importer and an actual distinct authorized reviewer, followed by genuine review/publication before the evidence deadline. This is separate from the now-implemented finite mapping and pricing-only preflight.

The source candidate and both new API operations are documented in [pricing](12-pricing-and-budget-guard.md#controlled-idr-coverage-candidate-2026-09-18) and OpenAPI. No database migration, role bypass, historical policy mutation or general paid-send authority is introduced. The existing pricing UI exposes candidate import and complete validation/diff evidence to reviewers.

Verification checkpoint: isolated PostgreSQL tests passed the real import/validate/diff/submit/distinct-review/approve/publish workflow using synthetic users and scoped currency proof. Complete published coverage produced IDR 356.65000000 with no budget, reservation, authorization, intent or attempt rows. Self-review, unauthorized publication/currency updates, wrong scope/market/currency and client allowance injection were denied. Existing interval tests cover one/two periods, exact boundary, gaps, stale review and rising/falling rates. Local verification completed: python scripts/dev.py up, python scripts/dev.py check and python scripts/dev.py e2e PASS (exit 0). The focused Inbox suite passed all 30 top-level tests (11 unit, 19 real PostgreSQL), including the new publication/preflight case. Full checks include Go vet/unit/build, actual PostgreSQL tenant/runtime/context-reuse and concurrency/security suites, frontend lint/typecheck/eight tests/build, OpenAPI and immutable migration checksums. Identity, Meta and Inbox browser suites all PASS with isolated databases and fake/local providers. Final source validation passed 185 local documentation links; git diff --check passed.

Dependency review: go mod verify PASS; pinned govulncheck v1.8.0 reports zero reachable vulnerabilities and zero in imported packages, with one advisory in a required module whose vulnerable package is unused. npm audit reports zero vulnerabilities. Local Windows tests were not race-instrumented; unchanged hosted CI must verify Linux race checks. No migration, CI, dependency lock, historical publication or operator-secret changes.

Hosted CI for this limited correction: PASS for [commit 4d2aff3](https://github.com/mbingsdk/whats/commit/4d2aff352e6ee372d96e7944aac86c54385d5a65), [run 35384998177](https://github.com/mbingsdk/whats/actions/runs/35384998177), completed 2026-09-18 UTC. All required stages executed and passed: isolated PostgreSQL, migration/repeat verification, source/SQL/Go/real PostgreSQL/frontend/OpenAPI checks including Linux race instrumentation, identity/Meta/Inbox browser flows, dependency integrity/vulnerability checks and cleanup. This result verifies the actual published correction; prior run 35051771598 only verifies the earlier evidence commit. This follow-up records the completed run and does not change implementation or acceptance authority. Review record: only the approved finite candidate, protected billing attestation, pricing-only preflight, import/evidence UI, API contracts, tests and these three evidence documents changed. Real coverage remains BLOCKED, acceptance OPEN, paid/template authority CLOSED, provider attempts 0 and Gate D CLOSED.
