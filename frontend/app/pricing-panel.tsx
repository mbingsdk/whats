...
export default function PricingPanel({org}:{org:Organization}){
 ...
 {can("pricing.registry.import")&&<button disabled={busy} onClick={()=>void act(async()=>{await request("/api/v1/rate-cards/imports","POST",{source:"SPRINT3_IDR_SERVICE_20260918",correction_reason:correction});})}>Import IDR Service coverage candidate</button>}
 <p>The IDR Service candidate covers July through December using the reviewed WABA timezone and quarterly pricing calendar. Inspect both effective intervals and source evidence before independent review. Importing or publishing does not approve spending.</p>
