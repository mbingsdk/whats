...
export default function Identity({screen}:{screen:Screen}){
 ...
 {organizations.length>1&&<label>Organization<select value={org?.id??""} onChange={e=>void act(async()=>{await request("/api/v1/auth/organization-session","POST",{organization_id:e.target.value});await refresh();})}><option value="">Select organization</option>{organizations.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label>}
